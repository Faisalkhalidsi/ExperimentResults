    ...
    public TDBN(ArrayList<URL> files) throws IOException {
        ...
        for (int i = 0; i < rbm1.length; i++)
            rbm1[i] = new RBM(ds.bodyParts[i].DOF, true,
                    demo.N_HIDDEN_UNIT_RBM, true);

        //////////////////////////////// Second RBM////////////////////////////////

        logger.fine("Initialize RBM...!");
        int hn = 0;
        ...
        rbm2 = new RBM(
                hn
                , true,
                demo.N_HIDDEN_UNIT_SECOND_RBM, false);
...
//
//        rbm21 = new RBM(
//                rbm2.hiddenLayer.size(), true,
//                demo.N_HIDDEN_UNIT_THIRD_RBM, false);

        //////////////////////////////// CRBM////////////////////////////////

        logger.fine("Initialize CRBM...!");

        rbm3 = new CRBM(rbm2.hiddenLayer.size(), false,
                demo.N_HIDDEN_UNIT_CRBM,false,degree);

        // mematikan hidden unit sehinggga berfungsi sebagai control terhadap jenis gerakan
        for (int i = 0; i < rbm3.hiddenLayer.size(); i++)
            rbm3.hiddenLayer.get(i).fixed = true;
    }
...
//    public void train21(int train_epoch) {
//        int mode, chosen;
//        for (RBM aRbm1 : rbm1) aRbm1.hiddenLayer.temp = 0.6;
...
//    }
    ...
    public void train3(int train_epoch) {
        int mode, chosen;
        for (RBM aRbm1 : rbm1) aRbm1.hiddenLayer.temp = 0.6;
        rbm2.hiddenLayer.temp = 0.6;
        ...
        while (epoch < train_epoch) {
            ...
            if (epoch > 5) rbm3.mom = 0.9;

            if (epoch % 2 == 0) {
                rbm3.visibleLayer.temp *= 0.99;
                rbm3.hiddenLayer.temp *= 0.99;
            }

            for (int t = 0; t < ds.getTotalFrameLength(); t++) {
                mode = rand.nextInt(ds.modeNum);
                chosen = rand.nextInt(ds.getFrameLength(mode) - step * (degree));
                initMode3(mode, chosen);
                double[] returnee = rbm3.train();
                energy += returnee[0];
                reconError += returnee[1];
            }
    ...
    public void initRealtime() {
        ...
        rbm3.visibleLayer.temp = 0.6;
        rbm3.hiddenLayer.temp = 0.6;
    ...
    public String[] stepRealtime(double control) {
        for (int i = 0; i < rbm3.hiddenLayer.size(); i++) {
            rbm3.hiddenLayer.get(i).val = control;
        }
...
//        }
    ...
    public void initMode3(int mode, int chosen) {
         for (int k = 0; k < degree + 1; k++) {
             rbm3.passValues();
             initMode2(mode, chosen);
             for (RBM aRbm1 : rbm1) aRbm1.hUpdate(false);
        ...
        if (mode < 5) {
            for (int i = 0; i < rbm3.hiddenLayer.size(); i++)
                rbm3.hiddenLayer.get(i).val = 0;
        } else {
            for (int i = 0; i < rbm3.hiddenLayer.size(); i++)
                rbm3.hiddenLayer.get(i).val = 1;
        }
    }
    ...
    void transferValuesUp3() {
        // from first RBM to second RBM
        int n = 0;
        for (RBM aRbm1 : rbm1) {
            int i = 0;
            while (i < aRbm1.hiddenLayer.size()) {
        ...
        //from second RBM to thrid RBM
        int j=0;
        while (j<rbm2.hiddenLayer.size()){
            rbm3.visibleLayer.get(j).val = rbm2.hiddenLayer.get(j).val;
            j++;
        }
    }
    ...
    void transferValuesDown3() {
        int j=0;
        while (j< rbm2.hiddenLayer.size()){
            rbm2.hiddenLayer.get(j).val = rbm3.visibleLayer.get(j).val;
            j++;
        }
